<script>
	...
	let copyTimeout;
	let discordCopySuccess = false;
	let discordCopyTimeout;
	...
	async function copyDiscordHandle() {
		try {
			await navigator.clipboard.writeText(profile?.discord_handle);
			discordCopySuccess = true;
			
			// Clear any existing timeout
			if (discordCopyTimeout) {
				clearTimeout(discordCopyTimeout);
			}
			
			// Reset the success state after 2 seconds
			discordCopyTimeout = setTimeout(() => {
				discordCopySuccess = false;
			}, 2000);
		} catch (error) {
			...
			try {
				const textArea = document.createElement('textarea');
				textArea.value = profile?.discord_handle;
				document.body.appendChild(textArea);
				textArea.select();
				document.execCommand('copy');
				document.body.removeChild(textArea);
				...
				console.error('Fallback Discord copy failed:', fallbackError);
			}
		}
	}
		...
		<div class="bg-gray-800 rounded-lg p-6 mb-8">
			<div class="flex flex-col md:flex-row items-start md:items-center gap-6">
				<div class="flex items-center gap-4">
					<!-- Extra large avatar without nickname -->
					<div class="w-36 h-36 rounded-full overflow-hidden flex-shrink-0 border border-gray-600">
						...
						{:else}
							<div class="w-full h-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
								<span class="text-white font-bold text-2xl">{walletAddress ? walletAddress.slice(2, 4).toUpperCase() : '??'}</span>
							</div>
						{/if}
					</div>
				</div>
						...
						<div class="flex items-center gap-3 mb-3">
							{#if profile?.twitter_handle}
								<a
									...
									class="inline-flex items-center gap-1 px-2 py-1 bg-black hover:bg-gray-800 text-white rounded text-xs transition-colors"
									title="Follow on X"
								>
							...
							{#if profile?.discord_handle}
								<button
									on:click={copyDiscordHandle}
									class="inline-flex items-center gap-1 px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs transition-colors duration-200 cursor-pointer"
