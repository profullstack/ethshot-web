-- Migration: Restore secure upsert_user_profile_secure function for ES256 JWT system
-- This migration restores the secure function that validates JWT tokens server-side

-- Drop the regular function if it exists
DROP FUNCTION IF EXISTS upsert_user_profile(text);

-- Create the secure function that validates JWT tokens
